...
void concat(SLinkedList<T> &l1, SLinkedList<T> &l2, SLinkedList<T> &lout)
{
    ...
    while (iterator)
    {
        std::cerr << "Iterator result: " << std::boolalpha << static_cast<bool>(iterator) << '\n'
                  << std::flush;
    ...
    //     while (!l2.empty()){
    //         dummy.addFront(l2.front());
    //         l2.removeFront();

    //     }

    //     while(!dummy.empty()){
    //         lout.addFront(dummy.front());
    //         dummy.removeFront();
    // }
}
...
int main()
{
    ...
    while (!p3.empty())
    { // Should print C C++ Java Python Javascript Go Rust Julia
        std::cout << p3.front() << " ";
        p3.removeFront();
    }
    std::cout << std::endl;
